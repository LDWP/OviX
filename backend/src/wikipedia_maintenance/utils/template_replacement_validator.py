...
class TemplateReplacementValidator:
    ...
    def validate(old_content: str, new_content: str,
                 ...
                 new_template: str,
                 normalize_name_func: Optional[Callable[[str], str]] = None) -> Tuple[bool, Optional[str]]:
        """
        Validate that template replacement is safe and minimal.
        ...
        Args:
            ...
            normalize_name_func: Optional function to normalize template names
                for comparison (e.g., to allow cite web → Lien web conversions).
                If provided, template names are normalized before comparison.
        ...
        if old_template_name and new_template_name:
            if normalize_name_func:
                ...
                if old_template_name.lower() != new_template_name.lower():
                    error_msg = f"Template validation failed: template name changed from {old_template_name} to {new_template_name}"
                    logger.warning(error_msg)
                    return False, error_msg
    ...
    @staticmethod
    def _extract_template_name(template: str) -> Optional[str]:
        """
