...
class CaseNormalizer:
    ...
    def _load_reference_data(self, config_path: Optional[str] = None) -> None:
        ...
        try:
            ...
            # Extract domain_to_site_name mapping from YAML format: "domain: [[Site Name]]"
            domain_mappings = data.get('domain_to_site_name', {}) or {}
            self.domain_to_site_name = {}
            for domain, wiki_links in domain_mappings.items():
                if wiki_links:
                    # Handle both list format [['Site Name']] and string format '[[Site Name]]'
                    if isinstance(wiki_links, list) and len(wiki_links) > 0:
                        ...
                        self.domain_to_site_name[str(domain)] = str(site_name)
                    else:
                        self.domain_to_site_name[str(domain)] = str(wiki_links)
    ...
    def _load_gemini_client(self) -> None:
        ...
        try:
            ...
            if not api_key:
                config_file = Path(__file__).parent.parent.parent.parent.parent / "config" / "config.yaml"
                try:
...
JSON normalisé :"""
    ...
    def _match_domain_mapping(self, value: str) -> Optional[str]:
        ...
        # Try with "www." prefix if it doesn't have one
        if not candidate.startswith('www.'):
            www_variant = f"www.{candidate}"
            if www_variant in self.domain_to_site_name:
                return self.domain_to_site_name[www_variant]
