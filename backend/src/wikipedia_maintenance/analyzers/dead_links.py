...
class DeadLinkAnalyzer(BaseAnalyzer):
    ...
    def _is_url_syntactically_valid(self, url: str) -> bool:
        ...
        Note: _load_academic_publisher_domains is cached with @lru_cache(maxsize=1)
        at module level in link_checker.py, so this is efficient and only loads once.
        """
    ...
    def _extract_site_name_from_url(self, url: str) -> Optional[str]:
        """
        Extract a MAPPED site name (wikilink) from a URL's domain.

        This method returns a value ONLY when the domain has a known mapping
        in the site-name YAML (i.e. the result is a wikilink like
        "[[Nom du site]]"). If the domain has no mapping, this returns None —
        it never falls back to the raw/unmapped domain. Callers rely on this
        contract to decide whether |site= should be written or corrected at
        all: an unmapped domain must never be written to a wikitext template.

        If the URL is an archive URL, first extract the original URL before
        determining the site name, so we get the original site instead of the
        archive site.
        ...
        Returns:
            A wikilink site name ("[[...]]") from the mapping, or None if the
        ...
        # CONTRACT: only accept a resolved wikilink. Some implementations of
        # _resolve_site_display_name may return the raw domain unchanged when
        # no mapping exists instead of None — guard against that here so an
        # unmapped domain never leaks through as a "site" value.
        if resolved and resolved.strip().startswith('[['):
            return resolved

        if resolved:
            logger.info(
                f"SITE_MAPPING_REJECTED | url={url} | domain={domain_for_lookup} | "
                f"resolved={resolved} | reason=not_a_wikilink_no_confirmed_mapping"
            )
        return None
    ...
    def _resolve_mapped_site_from_domain_string(self, domain_or_value: str) -> Optional[str]:
        """
        Resolve a bare domain/site string to a mapped wikilink, ONLY if a
        mapping is found. Returns None otherwise (never the raw string).

        Centralizes the "only if mapped" contract so every call site that
        tries to map a plain domain string (as opposed to a full URL) shares
        the exact same acceptance rule as _extract_site_name_from_url.
        """
        if not domain_or_value:
            return None
        resolved = self.reference_template_helper._resolve_site_display_name(domain_or_value)
        if resolved and resolved.strip().startswith('[['):
            return resolved
        return None
    ...
    def analyze(self, content: str) -> List[Issue]:
        ...
        # First filter for syntactically valid URLs, then apply max_checks_per_article
        # This ensures we actually check max_checks_per_article valid URLs, not raw candidates
        valid_matches = []
        for match in filtered_matches:
            url = match.group(0)
        ...
        # Apply max_checks_per_article limit to valid matches only
        matches_to_check = valid_matches[:self.max_checks_per_article]
        if len(valid_matches) > self.max_checks_per_article:
            analysis_complete = False
            logger.info(f"Reached max checks limit ({self.max_checks_per_article}), stopping with {len(valid_matches)} valid URLs available")

        # Parallel link checking with ThreadPoolExecutor
        if matches_to_check:
            logger.info(f"Starting parallel link check for {len(matches_to_check)} URLs with max_workers=5")
        ...
        for match in valid_matches:
            ...
            if template and template.parameters:
                ...
                if existing_archive_url:
                    ...
                    logger.info(f"EXISTING_ARCHIVE_DETECTED | url={url} | archive_url={existing_archive_url}")
                    ...
                    if existing_archive_status == LinkStatus.HEALTHY:
                        # CRITICAL FIX: Only add brisé le if the main link is actually DEAD
                        # This prevents adding brisé le to healthy links that happen to have a valid archive
                        if result.status == LinkStatus.DEAD:
                            ...
                            if template:
                                ...
                                # add_brise_le_parameter handles all variant checks internally
                            ...
                            # Link is not dead but has valid archive - skip adding brisé le
                            logger.info(f"SKIP_BRISE_LE_LINK_HEALTHY | url={url} | archive_valid={existing_archive_status.value} | main_link_status={result.status.value}")
                            continue
                    else:
                        ...
                        if archive_repair_result and archive_repair_result.decision == RepairDecision.REPLACEMENT_CONFIRMED:
                            ...
                            if template:
                                ...
                                new_template = self.reference_template_helper.generate_archive_repair_template(
                                    ...
                                    provider=provider,
                                    archive_title=archive_repair_result.details.get('archive_title')
                                )
                ...
                if other_providers:
                    ...
                    for alt_result in other_providers:
                        ...
                        if alt_check.status == LinkStatus.HEALTHY:
                            ...
                            # Recalculate metadata for the alternative provider (fix: keep title/author
                            # in sync with whichever provider actually ends up being used, instead of
                            # silently keeping the first provider's metadata).
                            alt_metadata = alt_result.metadata if alt_result else {}
                            archive_title = alt_metadata.get('title') if alt_metadata else None
                            archive_author = alt_metadata.get('author') if alt_metadata else None
                            logger.info(f"ARCHIVE_METADATA_UPDATED | url={url} | provider={alt_provider} | has_title={archive_title is not None} | has_author={archive_author is not None}")
    ...
    def _validate_template_replacement(self, old_content: str, new_content: str,
        ...
        Passes the template name canonicalization function to allow legitimate
        conversions (e.g., cite web → Lien web) while blocking unrelated changes.
        """
        is_valid, error_msg = self.template_validator.validate(
            old_content, new_content,
            old_template.start_position, old_template.end_position,
            new_template,
            normalize_name_func=self.reference_template_helper._get_canonical_template_name
        )
        return is_valid