...
class ReferenceEnricherAnalyzer(BaseAnalyzer):
    ...
    def _get_site_parameter_if_missing(self, template: ReferenceTemplate, url: str) -> Optional[str]:
        ...
        if current_site and current_site.strip() != "":
            ...
            # Check if current_site is a domain or URL that should be mapped
            # This includes cases like "lapresse.ca", "www.lapresse.ca", or full URLs
            if current_site and ('://' in current_site or current_site.strip().startswith('www.') or '.' in current_site.strip()):
                # Try to get the corrected version
                ...
                if '://' in current_site:
                    ...
                    # It's just a domain (with or without www)
                ...
                elif current_site.strip().startswith('www.') and domain != current_site.strip():
                    logger.info(
                        f"SITE_PARAMETER_CORRECTION | url={url} | template={template.template_name} | "
